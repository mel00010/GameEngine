...
class Dvd {
	public:
		template<typename Renderer> explicit Dvd(Renderer& renderer)
				: fs_(cmrc::dvd::get_filesystem()),
				logo_color_(glm::rgbColor(glm::vec3(glm::linearRand(0.0, 360.0), 1.0, 1.0))),
				dvd_texture_(renderer, fs_.open("dvd.png"), ShaderPrograms::DEFAULT, _3D::TextureType::DIFFUSE),
				vertices_({
						Vertex(glm::vec3( 0.5f,  0.5f, 1.0f), glm::vec3(0.0f, 0.0f, 0.0f), glm::vec2(1.0f, 1.0f)), // top right
						Vertex(glm::vec3( 0.5f, -0.5f, 1.0f), glm::vec3(0.0f, 0.0f, 0.0f), glm::vec2(1.0f, 0.0f)), // bottom right
						Vertex(glm::vec3(-0.5f, -0.5f, 1.0f), glm::vec3(0.0f, 0.0f, 0.0f), glm::vec2(0.0f, 0.0f)), // bottom left
						Vertex(glm::vec3(-0.5f,  0.5f, 1.0f), glm::vec3(0.0f, 0.0f, 0.0f), glm::vec2(0.0f, 1.0f))}), // top left
				indices_({0, 1, 3, 1, 2, 3}),
				handle_(renderer.GenerateVbo(ShaderPrograms::DEFAULT, vertices_, indices_)),
...
//			LOG_D("Starting DVD render");
			renderer.BindTexture(ShaderPrograms::DEFAULT, "texture_diffuse0", dvd_texture_, 0);
...
//			LOG_D("Finished DVD render");
		...
		void SetScale(const glm::dvec2 _scale) {
			scale_ = _scale;
		}

		void SetSpeed(const double _speed = 1.0) {
			speed_ = _speed;
		}
		void ModifySpeed(const double delta = 0.0) {
			speed_ *= (1.0 + delta);
		}
		double GetSpeed() {
			return speed_;
	...
	protected:
		...
		bool IsAtWindowEdge(const WindowEdge edge, const double length, const glm::dvec2 offset);

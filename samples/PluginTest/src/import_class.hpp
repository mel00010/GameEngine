...
inline std::unique_ptr<T, detail::deleter<T>> imported_class<T>::make_data(const smart_library& lib, Args ... args)
{
    ...
    std::cout << std::hex << *reinterpret_cast<uint32_t*>(&ctor.allocating) << std::endl;
    std::cout << std::hex << *reinterpret_cast<uint32_t*>(&ctor.standard) << std::endl;
    std::cout << std::hex << *reinterpret_cast<uint32_t*>(&dtor.deleting) << std::endl;
    std::cout << std::hex << *reinterpret_cast<uint32_t*>(&dtor.standard) << std::endl;

...
void Dvd::SetScale(const glm::dvec2 _scale) {
	scale_ = _scale;
}

void Dvd::SetSpeed(const float _speed) {
	speed_ = _speed;
}
void Dvd::ModifySpeed(const float delta) {
	speed_ *= (1.0 + delta);
}
double Dvd::GetSpeed() {
	return speed_;
...
void Dvd::CalculateMovement(const float fps) {
	...
	if(IsAtWindowEdge(WindowEdge::RIGHT, length, offset)) {
		delta_vec_ = glm::reflect(delta_vec_, glm::vec3(1.0, 0.0, 0.0) + glm::vec3(glm::diskRand(0.05), 0.0));
	...
	if(IsAtWindowEdge(WindowEdge::TOP, length, offset)) {
		delta_vec_ = glm::reflect(delta_vec_, glm::vec3(0.0, 1.0, 0.0) + glm::vec3(glm::diskRand(0.05), 0.0));
...
bool Dvd::IsAtWindowEdge(const WindowEdge edge, const float length, const glm::vec2 offset) {
	switch(edge) {
		case WindowEdge::RIGHT:		return length > (offset.x - dvd_.position_.x);
		case WindowEdge::LEFT:		return length > (offset.x + dvd_.position_.x);
		case WindowEdge::TOP:		return length > (offset.y - dvd_.position_.y);
		case WindowEdge::BOTTOM:	return length > (offset.y + dvd_.position_.y);
	}
	return false;
}

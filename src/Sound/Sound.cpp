...
Mix_Chunk* Sound::LoadSound(const cmrc::file file) {
  ...
  chunk_ = Mix_LoadWAV_RW(
      SDL_RWFromMem(file_contents.data(), file_contents.size()), 1);
  if (chunk_ == nullptr) {
    log_.Error("Mix_LoadWav:  {}", Mix_GetError());
  }
  return chunk_;
}
...
void Sound::Play(const size_t times) {
  ...
  if ((channel_ = Mix_PlayChannel(-1, chunk_, static_cast<int>(times - 1))) ==
      -1) {
    log_.Error("Mix_PlayChannel:  {}", Mix_GetError());
  }
}

...
bool ShaderProgram::Link() {
  ...
  if (isLinked == GL_FALSE) {
    ...
    glGetProgramInfoLog(program_, log_contents.capacity(), &log_size,
                        &log_contents[0]);
    log_.Error("Program linking failed!  Program log output follows:");
...
void ShaderProgram::UseProgram() const { glUseProgram(program_); }
void ShaderProgram::Use() const { UseProgram(); }

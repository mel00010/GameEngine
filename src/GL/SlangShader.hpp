...
class SlangShaderProgram {
	public:
		ShaderProgram* loadShaderProgram(const cmrc::file file) const {
			...
			//
			const SlangCompileRequest *slangRequest = spCreateCompileRequest(getSlangSession());
			...
			//
			spSetTargetProfile(slangRequest, targetIndex, spFindProfile(getSlangSession(), "glsl_460"));
			...
			int geometryIndex = spAddEntryPoint(slangRequest, translationUnitIndex, geometryEntryPointName, SLANG_STAGE_GEOMETRY);
			...
			if (auto diagnostics = spGetDiagnosticOutput(slangRequest)) {
				LOG_E("Slang output:" << diagnostics);
			}
			...
			spGetEntryPointCodeBlob(slangRequest, geometryIndex, 0, &geometryShaderBlob);

			// We extract the begin/end pointers to the output code buffers
			// using operations on the `ISlangBlob` interface.
			//
			char const *vertexCode = static_cast<char const*>(vertexShaderBlob->getBufferPointer());
			char const *vertexCodeEnd = vertexCode + vertexShaderBlob->getBufferSize();

			char const *fragmentCode = static_cast<char const*>(fragmentShaderBlob->getBufferPointer());
			char const *fragmentCodeEnd = fragmentCode + fragmentShaderBlob->getBufferSize();

			char const *geometryCode = static_cast<char const*>(geometryShaderBlob->getBufferPointer());
			char const *geometryCodeEnd = geometryCode + geometryShaderBlob->getBufferSize();

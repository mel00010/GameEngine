...
inline std::ostream& operator<<(std::ostream& os, GL gl) {
	return os << "GL {\n"<< push_indent
			<< "GLuint VAO = "<< static_cast<unsigned int>(gl.VAO) << "\n"
			<< "GLuint VBO = " << static_cast<unsigned int>(gl.VBO) << "\n"
			<< "GLuint EBO = " << static_cast<unsigned int>(gl.EBO) << "\n"
			<< pop_indent << "}";
}

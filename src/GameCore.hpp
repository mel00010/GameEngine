...
class GameCore {
	...
	public:
		...
		bool registerMouseButtonEventCallback(uint8_t button,
				ButtonEventType type,
				std::function<void(void)> callback);
		bool registerMouseWheelEventCallback(std::function<void(SDL_MouseWheelEvent&, ivec2 /* delta */)> callback);
	...
	protected:
		...
		std::list<std::function<void(SDL_MouseMotionEvent&, ivec2 /* pos */, ivec2 /* delta */)>> m_move_callbacks;
		std::array<std::list<std::function<void(SDL_MouseButtonEvent&, ivec2 /* pos */)>>, SDL_BUTTON_X2 + 1> m_button_up_callbacks;
		std::array<std::list<std::function<void(SDL_MouseButtonEvent&, ivec2 /* pos */)>>, SDL_BUTTON_X2 + 1> m_button_down_callbacks;
		std::array<std::list<std::function<void(void)>>, SDL_BUTTON_X2 + 1> m_button_held_callbacks;
		std::list<std::function<void(SDL_MouseWheelEvent&, ivec2 /* delta */)>> m_wheel_callbacks;

...
void Transformations::Rotate(const float delta_a, const float delta_b,
                             const float delta_c) {
  Rotate(glm::vec3(delta_a, delta_b, delta_c));
}
void Transformations::RotateA(const float a) { Rotate(a, 0.0, 0.0); }
void Transformations::RotateB(const float b) { Rotate(0.0, b, 0.0); }
void Transformations::RotateC(const float c) { Rotate(0.0, 0.0, c); }
...
void Transformations::RotateTo(const float rot_a, const float rot_b,
                               const float rot_c) {
  RotateTo(glm::vec3(rot_a, rot_b, rot_c));
}
void Transformations::RotateATo(const float a) { RotateTo(a, 0.0, 0.0); }
void Transformations::RotateBTo(const float b) { RotateTo(0.0, b, 0.0); }
void Transformations::RotateCTo(const float c) { RotateTo(0.0, 0.0, c); }
...
void Transformations::Move(const float delta_x, const float delta_y,
                           const float delta_z) {
  Move(glm::vec3(delta_x, delta_y, delta_z));
}
void Transformations::MoveX(const float x) { Move(x, 0.0, 0.0); }
void Transformations::MoveY(const float y) { Move(0.0, y, 0.0); }
void Transformations::MoveZ(const float z) { Move(0.0, 0.0, z); }
...
void Transformations::MoveTo(const float pos_x, const float pos_y,
                             const float pos_z) {
  MoveTo(glm::vec3(pos_x, pos_y, pos_z));
}
void Transformations::MoveXTo(const float x) { MoveTo(x, 0.0, 0.0); }
void Transformations::MoveYTo(const float y) { MoveTo(0.0, y, 0.0); }
void Transformations::MoveZTo(const float z) { MoveTo(0.0, 0.0, z); }
...
void Transformations::ScaleX(const float Scale) { ScaleXYZ(Scale, 1.0, 1.0); }
void Transformations::ScaleY(const float Scale) { ScaleXYZ(Scale, Scale, 1.0); }
void Transformations::ScaleZ(const float Scale) { ScaleXYZ(1.0, 1.0, Scale); }
...
void Transformations::ScaleXYZ(const float Scale_x, const float Scale_y,
                               const float Scale_z) {
...
void Transformations::ScaleXYZTo(const float Scale_x, const float Scale_y,
                                 const float Scale_z) {
  scaling_ = glm::vec3(Scale_x, Scale_y, Scale_z);
  model_ = glm::mat4(1.0f);
  model_ *= glm::orientate4(rotation_);

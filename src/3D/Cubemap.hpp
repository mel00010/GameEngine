...
struct CubemapBuffers {
 public:
  union {
    struct {
      uint8_t *positive_x, *negative_x, *positive_y, *negative_y, *positive_z,
          *negative_z;
    };
    struct {
      uint8_t *right, *left, *top, *bottom, *back, *front;
    };
  };
};
...
class Cubemap {
 public:
  ...
  Cubemap(const Renderer& renderer, const cmrc::embedded_filesystem& fs,
          const std::string& path, const ShaderPrograms shader_program);

  template <typename Renderer>
  GLuint LoadCubemap(const Renderer& renderer,
 ...
 protected:
  ...
  SDL_Surface* OpenImage(const CubeFace face,
                         const cmrc::embedded_filesystem& fs,
                         const std::string& path);

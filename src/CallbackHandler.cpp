...
bool CallbackHandler::RegisterKeyboardEventCallback(SDL_Scancode key,
	...
	if(type == KeyEventType::DOWN)	{ key_down_callbacks_[key].push_back(callback); }
	if(type == KeyEventType::UP)	{ key_up_callbacks_[key].push_back(callback); }
	return true;
}
bool CallbackHandler::RegisterKeyboardEventCallback(SDL_Scancode key,
				KeyEventType type,
				std::function<void(void)> callback) {
	if(type == KeyEventType::HELD) { key_held_callbacks_[key].push_back(callback); }
	return true;
}
...
bool CallbackHandler::RegisterMouseButtonEventCallback(uint8_t button,
	...
	if(type == ButtonEventType::DOWN)	{ m_button_down_callbacks_[button].push_back(callback); }
	if(type == ButtonEventType::UP)		{ m_button_up_callbacks_[button].push_back(callback); }
	return true;
}
bool CallbackHandler::RegisterMouseButtonEventCallback(uint8_t button,
		ButtonEventType type,
		std::function<void(void)> callback) {
	if(type == ButtonEventType::HELD) { m_button_held_callbacks_[button].push_back(callback); }
	return true;
}
...
bool CallbackHandler::RegisterJoyButtonEventCallback(
	...
	auto matcher = [button, type, this](SDL_Event& ev) -> bool {
		return	(type == ButtonEventType::DOWN && ev.type == SDL_JOYBUTTONDOWN && ev.jbutton.button == button) ||
				...
				(type == ButtonEventType::HELD && handler_.GetJoyButtonStatus(button));
	};
...
bool CallbackHandler::RegisterControllerButtonEventCallback(
	...
	auto matcher = [button, type, this](SDL_Event& ev) -> bool {
		return	(type == ButtonEventType::DOWN && ev.type == SDL_CONTROLLERBUTTONDOWN && ev.cbutton.button == button) ||
				...
				(type == ButtonEventType::HELD && handler_.GetControllerButtonStatus(button));
	};
...
bool CallbackHandler::RegisterTimeoutCallback(std::string identifier, size_t ms, std::function<void(void)> callback, bool repeat) {
	if(std::any_of(timeout_callbacks_.begin(), timeout_callbacks_.end(), [&identifier](TimeoutCallback c) { return c.identifier == identifier; })) {return false; }
	timeout_callbacks_.emplace_back(TimeoutCallback { identifier, SDL_GetTicks(), ms, repeat, callback, } );
	return true;
}
bool CallbackHandler::UnregisterTimeoutCallback(std::string identifier) {
	timeout_callbacks_.erase( std::remove_if (timeout_callbacks_.begin(), timeout_callbacks_.end(),
		[&identifier](TimeoutCallback c) { return c.identifier == identifier; }), timeout_callbacks_.end());
	return false;
}

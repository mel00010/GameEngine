...
class InputHandler {
 public:
   ...
   * @param status Boolean representing the key's status according to this
   * table: | State | Key status  |
   * 		  |-------|-------------|
   * 		  | True  | Pressed     |
   * 		  | False | Not pressed |
   */
   ...
   * @param status Boolean representing the button's status according to this
   * table: | State | Button status  |
   * 		   |-------|----------------|
   * 		   | True  | Pressed        |
   * 		   | False | Not pressed    |
   */
 ...
 private:
   ...
   */
  std::array<bool, SDL_CONTROLLER_BUTTON_MAX> controller_buttons_status_ = {
      false};
  /**
   * @brief An array containing the status of the joystick buttons
   */
  std::array<bool, std::numeric_limits<uint8_t>::max()>
      joystick_buttons_status_ = {false};
  /**

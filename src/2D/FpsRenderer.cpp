...
void FpsRenderer::CalculateFps() {
	...
	fps_ = static_cast<double>(frames_rendered_) * static_cast<double>(ticks_per_second_);
	fps_avg_ = std::accumulate(fps_avg_array.begin(), fps_avg_array.end(), 0) / 20;
...
void FpsRenderer::CalculateFrameTime() {
	...
	time_avg_array[pos] = std::chrono::duration_cast<std::chrono::nanoseconds>(frame_stop_time_ - frame_start_time_).count();
	if( pos >= 19) {
		pos = 0;
	} else {
		pos++;
	}
	frame_time_ns_ = std::accumulate(time_avg_array.begin(), time_avg_array.end(), 0) / 20.0;
	frame_time_us_ = frame_time_ns_/1000.0;
	frame_time_ms_ = frame_time_ns_/1000000.0;
	frame_time_s_ =  frame_time_ns_/1000000000.0;
}

void FpsRenderer::SetShowFps(const bool enable) {
	fps_shown_ = enable;
}
...
void FpsRenderer::ToggleFps() {
	fps_shown_ = !fps_shown_;
	LOG_D("Toggled FPS display " << (fps_shown_ ? "on" : "off"));
}

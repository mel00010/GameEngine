...
class Renderer : public W_Derived, Crtp<Renderer, R_Derived, W_Derived> {
	public:
		...
		void BindTexture(const ShaderPrograms shader_program, const std::string& name,
			...
			this->Underlying().BindTexture(name, texture, texture_unit);
		}
		/**
		...
		void BindCubemap(const ShaderPrograms shader_program, const std::string& name,
			...
			this->Underlying().BindCubeMap(name, cube_map, texture_unit);
		}
		...
		void EnableBlending() const {
			this->Underlying().EnableBlending();
		}
		/**
		 * @brief Disable blending
		 */
		void DisableElending() const {
			this->Underlying().DisableBlending();
		}
		/**
		 * @brief Enable depth testing
		 */
		void EnableDepthTesting() const {
			this->Underlying().EnableDepthTesting();
		}
		/**
		 * @brief Disable depth testing
		 */
		void DisableDepthTesting() const {
			this->Underlying().DisableDepthTesting();
		}
		...
		unsigned int CreateTexture(const ShaderPrograms shader_program, const _3D::PixelFormat format,
			...
			return this->Underlying().CreateTexture(format, size, pixels);
		}
		/**
		...
		unsigned int CreateCubemap(const ShaderPrograms shader_program, const _3D::PixelFormat format,
			...
			return this->Underlying().CreateTexture(format, size, buffers);
		}
		/**
		...
		void SetSwizzleMask(const GLint swizzle_r, const GLint swizzle_g,
			...
			this->Underlying().SetSwizzleMask(swizzle_r, swizzle_g, swizzle_b, swizzle_a);
		}
